...
pkgs.writeShellScriptBin "monitor-hotplug" ''
  ...
  if command -v ${pkgs.mons}/bin/mons >/dev/null 2>&1; then
      ...
      connected_monitors=$(${pkgs.xrandr}/bin/xrandr --query | ${pkgs.gnugrep}/bin/grep " connected" | ${pkgs.coreutils}/bin/cut -d" " -f1)

      if [ -n "$connected_monitors" ]; then
          # Get internal monitor name (usually eDP-1, eDP-0, etc.)
          internal_monitor=$(${pkgs.xrandr}/bin/xrandr --query | ${pkgs.gnugrep}/bin/grep -E " connected" | ${pkgs.gnugrep}/bin/grep -E "eDP-[0-9]|LVDS-[0-9]|eDP1|LVDS1" | ${pkgs.coreutils}/bin/head -n1 | ${pkgs.coreutils}/bin/cut -d" " -f1)
          ...
          if [ -n "$internal_monitor" ] && [ -n "$(echo "$connected_monitors" | ${pkgs.gnugrep}/bin/grep "$internal_monitor")" ]; then
              ...
              primary_monitor=$(echo "$connected_monitors" | ${pkgs.coreutils}/bin/head -n1)
          ...
          internal_display=$(${pkgs.xrandr}/bin/xrandr --query | ${pkgs.gnugrep}/bin/grep -E "eDP-[0-9]|LVDS-[0-9]|eDP1|LVDS1" | ${pkgs.coreutils}/bin/head -n1 | ${pkgs.coreutils}/bin/cut -d" " -f1)
          if [ -n "$internal_display" ]; then
              ${pkgs.xrandr}/bin/xrandr --output "$internal_display" --auto --primary
          fi
      fi
